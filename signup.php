...
// Prepare SQL statement to check if the username or email already exists in guest table
$query_guest = $conn->prepare("SELECT * FROM guest WHERE username=? OR email=?");
$query_guest->bind_param("ss", $username, $email);
$query_guest->execute();
$result_guest = $query_guest->get_result();

if ($result_admin->num_rows > 0 || $result_guest->num_rows > 0) {
...
// Insert user data into the guest table
$query_guest_insert = $conn->prepare("INSERT INTO guest (username, email, phone_no, passwd) VALUES (?, ?, ?, ?)");
$query_guest_insert->bind_param("ssss", $username, $email, $phone_no, $hashedPassword);
$query_guest_insert->execute();

// Check if both inserts were successful
if ($query_admin_insert->affected_rows > 0 && $query_guest_insert->affected_rows > 0) {
