...
class MyHTTPRequestHandler(SimpleHTTPRequestHandler):
    def do_POST(self):
        ...
        # Format destination location code
        def get_destination_location_code(location_input):
            # Extract the first three letters and convert to uppercase
            destination_location_code = location_input[:2].upper()
            return destination_location_code
        # Format destination location code
        destination_location_code = get_destination_location_code(location_input)
        ...
        # Ensure non-stop attribute is boolean
        non_stop = parsed_data.get("non_stop", False)
        if isinstance(non_stop, str):
            non_stop = non_stop.lower() == "true"
        
        logging.info(f"Non-Stop Attribute: {non_stop}")
        ...
        def get_hotel_recommendations(hotel_features, location_input):
            ...
            # Train KNN model
            knn_model = NearestNeighbors(n_neighbors=15, algorithm='brute', metric='euclidean')
            ...
            for index in indices[0]:
                ...
                if location:
                    # Fetching weather data for the hotel's location
                    ...
                    recommended_hotels.append({
                        ...
                        "reviews": reviews,
                        "weather": weather  # Include weather information
                    })
        ...
        # Function to fetch weather data for a location using OpenWeatherMap API
        def get_weather(lat, lon):
            api_key = 'cc8c35a9faa3f227d129ff1661e172d7'
            ...
            if response.ok:
                ...
                return weather
            logging.error(f"Error fetching weather data for coordinates ({lat}, {lon}) - {response.status_code} {response.reason}")
            return {"temperature": "N/A", "description": "N/A", "icon": ""}
